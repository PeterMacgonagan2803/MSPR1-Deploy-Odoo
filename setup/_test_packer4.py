"""Test Packer v4 - sans cloud_init=true (evite conflit IDE)."""
import paramiko
import sys
import time
...
# Regenerer hash si placeholder present
grep -q '__UBUNTU_HASH__' packer/http/user-data && {
    ...
    echo "Hash regenere"
} || echo "Hash deja present"

echo "Config Packer:"
grep -E 'cloud_init|additional_iso|boot_command|ds=' packer/ubuntu-k3s.pkr.hcl
echo "PREP_OK"
""", label="PULL + PREP")

print("\n=== PACKER BUILD v4 (sans cloud_init, CD-ROM) ===", flush=True)
t = time.time()
...
packer build \
    ...
    -var "vm_id=9001" \
    . 2>&1 | tail -100
""", timeout=1800, label="PACKER BUILD v4")
...
qm config 9001 2>/dev/null | grep -E 'ide|scsi|name|cloud' || true
""", label="VERIFY")
