...
- [ ] **Alternative** -- Deploiement automatique complet (deploy-all.py)
...
|-------------|---------|---------------|
| **IP publique** | `51.77.216.79` | Email OVH ou `ip a show vmbr0` |
...
```hcl
# --- Connexion Proxmox ---
proxmox_url      = "https://51.77.216.79:8006"
...
```powershell
Add-Content -Path "C:\Windows\System32\drivers\etc\hosts" -Value "`n51.77.216.79  odoo.local"
```

Verification :
```powershell
ping odoo.local
# Doit resoudre vers 51.77.216.79
```
...
Le script `setup/deploy-all.py` automatise **TOUTES** les etapes ci-dessus (sauf l'etape 0 et la configuration initiale du reseau Proxmox) en une seule commande.
...
|-------|--------|---------------|
| 1/7 | Reset complet Proxmox (VMs, template, iptables) | ~15s |
| 2/7 | Creation du template VM cloud-init | ~2min |
| 3/7 | Terraform apply (4 VMs + inventaire) | ~2min |
| 4/7 | Setup Ansible (install, clone, SSH keys, Galaxy) | ~3min |
