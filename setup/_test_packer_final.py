"""Test Packer FINAL - git reset hard + user-data simplifie + HTTP + kernel ip=."""
import paramiko
import time
...
# Cleanup VM
ssh_run("qm stop 9001 --timeout 10 2>/dev/null; qm destroy 9001 --purge 2>/dev/null; echo CLEAN", label="CLEANUP VM")

# Reset repo + generate hash + write user-data
ssh_run(r"""
...
echo ""
echo "=== packer hcl (boot) ==="
grep -E 'http_|boot_command|ip=|ssh_host' packer/ubuntu-k3s.pkr.hcl
echo "PREP_OK"
""", label="RESET REPO + HASH")

# Build
print("\n=== PACKER BUILD FINAL ===", flush=True)
t = time.time()
